...
static int nrm_net_send(struct nrm_context *ctxt, char *buf,
...
int nrm_init(struct nrm_context *ctxt, const char *task_id)
{
    ...
    /* app init */
    char buf[512];
    snprintf(buf, 512, NRM_START_FORMAT, ctxt->cmd_id, ctxt->task_id);
...
int nrm_fini(struct nrm_context *ctxt)
{
    ...
    assert(ctxt != NULL);
    if (ctxt->acc != 0) {
      snprintf(buf, 512, NRM_PROGRESS_FORMAT, ctxt->acc, ctxt->task_id);
      err = nrm_net_send(ctxt, buf, 512, 0);
    }
    snprintf(buf, 512, NRM_EXIT_FORMAT, ctxt->task_id);
    err = nrm_net_send(ctxt, buf, 512, 0);
    assert(err > 0);
    free(ctxt->task_id);
    nrm_net_fini(ctxt);
    ...
    unsigned long long int computeTime)
{
    ...
    if(timediff > nrm_ratelimit_threshold)
    {
        snprintf(buf, 512, NRM_PHASECONTEXT_FORMAT, cpu, (unsigned int)(ctxt->acc),
            computeTime, timediff, ctxt->task_id);
        int err = nrm_net_send(ctxt, buf, 512, ZMQ_DONTWAIT);  
	if(err == -1)
	{
	    assert(errno == EAGAIN);
...
long long int nrm_timediff(struct nrm_context *ctxt, struct timespec
		end_time)
{
