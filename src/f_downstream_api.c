...
int FC_FUNC_(f_nrm_init, F_NRM_INIT)(uintptr_t *ctxt, char* uuid_in, int* size)
{
...
int FC_FUNC_(f_nrm_send_progress, F_NRM_SEND_PROGRESS)
    (uintptr_t *ctxt, unsigned long *progress)
{
    return nrm_send_progress(*((struct nrm_context **)ctxt), *progress);
}

int FC_FUNC_(f_nrm_send_phase_context, F_NRM_SEND_PHASE_CONTEXT)
    (uintptr_t *ctxt, unsigned int *cpu, unsigned long long int *computeTime)
{
